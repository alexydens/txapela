...
/* Globals */
static struct __packed __aligned(0x10) gdt_entry {
  u16 limit_low;
  u16 base_low;
  u8 base_middle;
  u8 access;
...
static struct __packed __aligned(0x10) gdtr {
  u16 limit;
  u64 base;
} gdtr ;
static struct __packed __aligned(0x10) tss {
  u32 reserved0;
  u64 rsp0;
  u64 rsp1;
  u64 rsp2;
...
bool gdt_init(void) {
  ...
  /* Set the TSS */
  __asm__ __volatile__ ("ltr %0" : : "r"((u16)GDT_SEG_TSS));
